...
class Room {
  constructor(id, tableType, name,tableNumber,dealer, minPlayers, maxPlayers, type, owner,gameType, rack, entryFees, currentPlayer, players, gameWinners, gameLosers,status,  game,varient, numberOfDecks ,  printedJoker,timerStart,gameOverPoint, gamecount) {
    ...
    if (players && Array.isArray(players)) {
        ...
        players.forEach(function (player) {
               room.players.push(new Sys.Game.Practice.Deals.Entities.Player().createObject(player));
        })
    }
    ...
    this.game = null
    if (game) {
      this.game = new Sys.Game.Practice.Deals.Entities.Game().createObject(game);
  ...
  toJson() {
    ...
    // other field
    if (this.game) {
      // room.game = new Sys.Game.Practice.Deals.Entities.Game(this.game.id,this.game.roomId,this.game.status, this.game.pot, this.game.history, this.game.gameNumber, this.game.gameType, this.game.jokerCard, this.game.openDeck, this.game.closeDeck, this.game.finishDeck, this.game.declare);
  ...
  async AddPlayer(id, socketId, username, status, avatar, appId, chips,cash, seatIndex, isBot) {
    ...
    let player = new Sys.Game.Practice.Deals.Entities.Player().createObject(plyObj);

    // let player = new Sys.Game.Practice.Deals.Entities.Player(id,socketId,username,avatar,status,appId,chips,cash,80,gameOverPoints,0,'-',seatIndex,isBot,true,false,false,0,1,[],0,Sys.Config.Rummy.extraTime,0);
  ...
  async StartGame() {
    ...
    if (!room.game && playersLength >= this.minPlayers) {
      ...
      let game = await Sys.Game.Practice.Deals.Services.GameServices.create(gameobj);
      ...
      if (game) {
        ...
        if (playersLength >= room.minPlayers) {
          ...
          let gameStarted = await Sys.Game.Practice.Deals.Controllers.RoomProcess.newGameStarted(room);
          room.initNewRound();
        }
      }
    }
  ...
  async initNewRound() {
    ...
    let deck =  new Sys.Game.Practice.Deals.Entities.Deck();
   ...
   // //console.log("room.game.openDeck ->",room.game.openDeck);
    ...
    if (room.gamecount == 1) {
      let priorityCards = await Sys.Game.Practice.Deals.Controllers.RoomProcess.turnPlayerSelection(room,cardsArray);
    ...
    Sys.Timers[room.id] = setTimeout(async function(room){
      ...
      await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('PlayerList', playerInfoDummy, { isBeforeGameStart: roomStatus });
     room.NewRound();
    }, 5000, room);
  }
  ...
  async NewRound() {
    ...
    room.status = 'Running';
     room = await Sys.Game.Practice.Deals.Services.RoomServices.update(room);
    let newRoundStarted = await Sys.Game.Practice.Deals.Controllers.RoomProcess.newRoundStarted(room);
