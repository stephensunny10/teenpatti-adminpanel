...
module.exports = {
    checkRoomSeatAvilability: async function(data){
        try {
            ...
            if (isEmptySpace == false) {
                ...
                let rm = await Sys.Game.Practice.Deals.Services.RoomServices.create(data);
                if (!rm) {
                    return { status: 'fail', result: null, message: 'No Room Created.', statusCode: 401 }
                }
                room = rm; // Assign to Room
		    }
            room = await Sys.Game.Practice.Deals.Services.RoomServices.get(room.id); //// Just Get Table Data With Format.
    ...
    joinRoom: async function(player,data){
        try {
            ...
            let room = await Sys.Game.Practice.Deals.Services.RoomServices.get(data.roomId);
            if (!room) {
    ...
    for (let i = 0; i < room.players.length; i++) {
      ...
      // When Fist User Wants to Push on Table.
		...
		if (seatAvailable) {
			...
			// Player chips Cut
			// let newChips =  parseInt(parseInt(player.chips) - parseInt(data.entryFees));
      // if (parseInt(data.entryFees) > player.chips) {
      //   newChips = 0;
      // }
			// let updatedPlayer = await Sys.Game.Practice.Deals.Services.PlayerServices.update(player.id,{ chips: newChips });
			// let trnsObj = {
			// 	playerId : player.id,
			...
			// 	status           : 'sucess',
			// }
			// let chipsTransection = await Sys.Game.Practice.Deals.Services.PlayerServices.cerateChipsCashHistory(trnsObj);
        ...
        if (oldPlayer.cards.length > 13) {
          oldPlayer.cardTurn  = true;
				...
				roomUpdated = await Sys.Game.Practice.Deals.Services.RoomServices.update(room);
        ...
        await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(data.socketId).emit('PlayerList', playerInfoDummy, { isBeforeGameStart: roomStatus });
        await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('JoinedPlayerInfo',joinedPlayer, { isBeforeGameStart: roomStatus });
					...
					if (room.status != 'Running' && playersLength >= room.minPlayers) {
						...
						if (room.game == null && room.timerStart == false) {
							room.timerStart = true; // When 12 Second Countdown Start.
              				room = await Sys.Game.Practice.Deals.Services.RoomServices.update(room);
							...
							let	timer = parseInt(Sys.Config.Rummy.waitBeforeGameStart);
								Sys.Timers[room.id] = setInterval(async function(room){
									...
									await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('OnGameStartWait', {
										roomId: room.id,
										timer : timer,
    ...
    try {
  		...
  		await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('GameStarted', {
 	...
 	turnPlayerSelection: async function(room,cardsArray) {
        ...
        let player = await Sys.Game.Practice.Deals.Services.PlayerServices.getById(room.players[i].id);
      ...
      //console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
      //console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
			await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('priorityCards', sellectObj);
			return room;
		}catch (error) {
	...
	newRoundStarted: async function(room) {
      ...
      //console.log('<=> Game || newRoundStarted :',room.game.gameNumber);
			...
			await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('JokerOpenCardInfo', joObj);
			...
			room.players.forEach(async function (player) {
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('PlayerDeck', playerCards);
			});
			// Send Second Timer Player Deck for Testing.
			room.players.forEach(async function (player) {
        // ~~~
				// Cut Player Cheip When Card Distribution
				// let playerObj = await Sys.Game.Practice.Deals.Services.PlayerServices.getById(player.id);
				// let newChips = parseInt(playerObj.chips)-parseInt(room.entryFees);
				// let updatedPlayer = await Sys.Game.Practice.Deals.Services.PlayerServices.update(playerObj.id,{ chips: newChips });
			...
			Sys.Timers[room.id] = setTimeout(async function (room) {
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('TurnPlayer', {
					...
					Sys.Timers[room.id] = setInterval(async function(room){
						...
						await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('OnTurnTimer', {
							playerId: room.getCurrentPlayer().id,
							timer : timer,
						...
						if(timer < 1){
							...
							if(extraTimer < 1){
								...
								Sys.Game.Practice.Deals.Controllers.RoomProcess.playerDefaultAction(room.id);
							}else{
								Sys.Timers[room.id] = setInterval(async function(room){
    ...
    try {
		...
		let room = await Sys.Game.Practice.Deals.Services.RoomServices.get(id);
		...
		if (room.getCurrentPlayer()) {
			...
			if(currentPlayer.droppedCount == 2){
				...
				// 	roomId : room.id
				// }
				// let responce =  await Sys.Game.Practice.Deals.Controllers.RoomProcess.leftRoom(playerData);
        ...
        // currentPlayer.playerScore = parseInt(parseInt(currentPlayer.playerScore) - 80);
        ...
        currentPlayer.dropped = true;
				room = await Sys.Game.Practice.Deals.Controllers.RoomProcess.saveHistory(room,dataObj);
      ...
      // else{
				...
				if(playercards == 14){
					...
					await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('OnAutoDiscard', {
						playerId: currentPlayer.id,
						card : card[0],
					});
					//Save History
				...
				roomUpdated = await Sys.Game.Practice.Deals.Services.RoomServices.update(room);
				await Sys.Game.Practice.Deals.Controllers.RoomProcess.changeTurn(room);
				await Sys.Game.Practice.Deals.Controllers.RoomProcess.turnFinished(room);
			// }
    ...
    try {
				   ...
				   if (room.players[i].id == data.playerId && room.players[i].status != 'Left') {
					   ...
					   if(room.players[i].status == 'sitting' || room.players[i].status == 'waiting'){
						   removePlayer = true;
							...
							let playerObj = await Sys.Game.Practice.Deals.Services.PlayerServices.getById(data.playerId);
							if(!playerObj){
								return { status: 'fail', result: null, message: 'No Player Found!.', statusCode: 401 }
							}

							let newChips =  parseInt(parseInt(playerObj.chips) + parseInt(room.entryFees));
							let updatedPlayer = await Sys.Game.Practice.Deals.Services.PlayerServices.update(playerObj.id,{ chips: newChips });
							let trnsObj = {
		   ...
		   if (player) {
			   ...
			   if (room.game && room.game.status == 'Running') {
				   ...
				   room  = await Sys.Game.Practice.Deals.Controllers.RoomProcess.checkTurnFinished(room,playerId);
		   ...
		   await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('PlayerLeft', { 'playerId': player.id });

		  //  room  =  await Sys.Game.Practice.Deals.Controllers.RoomProcess.broadcastPlayerInfo(room);
			   return {
				   status: 'success',
	...
	// 	await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('playerInfoDummy', playerInfoDummy,{vacantSeat: playerFreeSeats});
	// 	return room;
	// }catch (error) {
    ...
    try {
          ...
          room = await Sys.Game.Practice.Deals.Services.RoomServices.update(room);
  				...
  				await Sys.Game.Practice.Deals.Controllers.RoomProcess.gameFinished(room);
							...
							Sys.Timers[room.id] =  setInterval(async function(room){
								...
								if(timer < 1){
									clearTimeout(Sys.Timers[room.id]); // Clear Turn Timer
										...
										//console.log("Turn - check is timer not Close...");
										await Sys.Game.Practice.Deals.Controllers.RoomProcess.playerDefaultAction(room.id);
    ...
    try {
  		...
  		for (let i=0; i < room.players.length; i += 1) {
  			// room.players[i].totalPoint = parseInt(room.players[i].totalPoint) - parseInt(room.players[i].playerScore);
        ...
        // //console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
            ...
            let FirstWinner = room.getPlayerById(room.game.declare[i]);
  					...
  					// 	points : parseInt(room.players[i].playerScore),
  					// 	totalPoint : parseInt(room.players[i].totalPoint) + parseInt(room.players[i].playerScore),// For Pool / Deal
      			// 	dropped : room.players[i].dropped
  					// });
            winnerPlayers.push({
  						type : 'winner',
  						...
  						points : parseInt(FirstWinner.playerScore), // not Use in Pool
  						totalPoint : parseInt(FirstWinner.totalPoint) + parseInt(FirstWinner.playerScore),// For Pool / Deal
          ...
          // //console.log("222222222222222222222222222222222222222222222222222222222");
            winnerPlayers[0].points = parseInt(room.players[i].playerScore)
            winnerPlayers[0].totalPoint = winnerPlayers[0].totalPoint + parseInt(room.players[i].playerScore)
            room.getPlayerById(winnerPlayers[0].playerId).totalPoint = winnerPlayers[0].totalPoint
  					winnerPlayers.push({
  						...
  						cards : room.players[i].cardsString,
  						points : parseInt(room.players[i].playerScore),
  						totalPoint : (parseInt(room.players[i].totalPoint) < 0 ) ? 0 : parseInt(room.players[i].totalPoint) ,// For Pool / Deal
        			dropped : room.players[i].dropped,
          ...
          // if (room.game.declare.indexOf(room.players[i].id) == -1 && room.players[i].status != 'waiting' ) { // if Player not in Declare Array
  				if (room.players[i].status != 'waiting' ) { // if Player not in Declare Array
  					room.players[i].totalPoint = parseInt(room.players[i].totalPoint) - parseInt(room.players[i].playerScore); // Update Player Total Points

            if (room.players[i].id != winnerPlayers[0].playerId) {
              // ~~ Comment this in case of 6 players playing Deal game
                winnerPlayers[0].points = parseInt(room.players[i].playerScore)
                winnerPlayers[0].totalPoint = parseInt(winnerPlayers[0].totalPoint) + parseInt(room.players[i].playerScore)
                room.getPlayerById(winnerPlayers[0].playerId).totalPoint = parseInt(winnerPlayers[0].totalPoint)
                ...
                // //console.log("33333333333333333333333333333333333333333333333333333333");
    					...
    					winnerPlayers.push({
    						...
    						score : 0,
    						cards : room.players[i].cardsString,
    						points : parseInt(parseInt(room.players[i].playerScore) * (-1)), // not Use in Pool
    						totalPoint : (parseInt(room.players[i].totalPoint) < 0 ) ? 0 : parseInt(room.players[i].totalPoint) ,
         				dropped : room.players[i].dropped,
                wrongfinished: room.players[i].wrongfinished
        ...
        // winner.totalPoint = parseInt(winner.totalPoint) + parseInt(winner.playerScore); // Update Player Total Points
        ...
        // //console.log("444444444444444444444444444444444444444444444444444444444444");
          ...
          points : parseInt(winner.playerScore),   // For Pool / Deal
  				// points : 0,   // For Pool / Deal
  				totalPoint : parseInt(winner.totalPoint),
      		dropped : winner.dropped,
          wrongfinished: winner.wrongfinished
  			});
  			// Add Seingle Winner Chips
  			// let playerObj = await Sys.Game.Practice.Deals.Services.PlayerServices.getById(winner.id);
  			// let newChips = parseInt(playerObj.chips)+parseInt(singlePlayerWinnigchips) ;
  			// let updatedPlayer = await Sys.Game.Practice.Deals.Services.PlayerServices.update(winner.id,{ chips: newChips });
            ...
            //console.log('winnerPlayers[0].totalPoint before',winnerPlayers[0].totalPoint);
  					// winnerPlayers[0].totalPoint = parseInt(winnerPlayers[0].totalPoint) + parseInt(room.players[i].playerScore); // Update Winner Total Points
            // room.getPlayerById(winnerPlayers[0].playerId).totalPoint = parseInt(winnerPlayers[0].totalPoint)
            ...
            // //console.log('winnerPlayers[0].totalPoint after',winnerPlayers[0].totalPoint);
  					...
  					winnerPlayers.push({
  						...
  						username : room.players[i].username,
  						won : 0,
  						score : 0, // parseInt(room.players[i].playerScore)
  						cards : room.players[i].cardsString,
  						points : parseInt(parseInt(room.players[i].playerScore) * (-1)),  // Not use in Pool
  						totalPoint : (parseInt(room.players[i].totalPoint) < 0 ) ? 0 : parseInt(room.players[i].totalPoint) ,
   				    dropped : room.players[i].dropped,
  		...
  		let timer = Sys.Config.Rummy.waitBeforeGameRestart;
  		let totalPlayers = 0;
  		for (i = 0; i < room.players.length; i++) {
  			if(room.players[i].status != 'Left'){
  				totalPlayers++;
      ...
      //     winnerPlayers[0].totalPoint = parseInt(winnerPlayers[0].totalPoint + getPlayerById(winnerPlayers[1].playerId).playerScore)
      ...
      // }
  		...
  		Sys.Timers[room.id] = setInterval(async function(room){
  			...
  			await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('OnGameFinished', {
  			...
  			if(timer < 1){
  				...
  				await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('OnGameRestart', {});

  				for (i = 0; i < room.players.length; i++) {

  					// check If Player total Point is More then GameOverPoint
  					if(parseInt(room.players[i].totalPoint) <= 0){
  						...
  						room.players[i].status = 'Left';
  						await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('OnPoolDealLeave', {
  							playerId : room.players[i].id,
          ...
          let currentRoomId = room.game.id;
                ...
                if (totalPlayers < room.minPlayers || room.gamecount == room.gameOverPoint) {
                  restartGame = false;
                  let minTotalPoint = parseInt(room.gameOverPoint) * 80;
                  for (let i=0; i < room.players.length; i += 1) {
                    ...
                    for (let j=0; j < room.players.length; j += 1) {
                      if (room.players[i].id != room.players[j].id && room.players[i].totalPoint == room.players[j].totalPoint) {
                        if (room.players[j].totalPoint == minTotalPoint) {
                          ...
                          await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('TieMatch', {});
                          restartGame = true;
                          tieMatch = true
                        }
                      }
                  ...
                  //   let updatedAdminCommission = await Sys.Game.Practice.Deals.Services.RoomServices.createCommission({
                  //     roomid                : room.id,
                  ...
                  // }
                    ...
                    // //console.log("totalWiningchipstotalWiningchipstotalWiningchipstotalWiningchips4", totalWiningchips);
  									let playerObj = await Sys.Game.Practice.Deals.Services.PlayerServices.getById(poolwinner.playerId);
  									// Add Player Chips
  									let newChips = parseInt(parseInt(playerObj.chips) + parseInt(totalWiningchips));
                    // //console.log("playerObj.chips", playerObj.chips);
                    // //console.log("totalWiningchipstotalWiningchipstotalWiningchipstotalWiningchips5", totalWiningchips);
  									let updatedPlayer = await Sys.Game.Practice.Deals.Services.PlayerServices.update(playerObj.id,{ chips: newChips });
                      ...
                      gameId            : currentRoomId,
  								...
  								await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('OnPoolDealGameWinner', poolwinner);
	...
	pushOpenDeck: async function(data) {
      ...
      if (room.status != 'Running') {
			...
			if (room.game.status == 'Running') {
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('PushOpenDeck',broObj);
	...
	popOpenDeck: async function(data) {
      ...
      if (room.status != 'Running') {
        ...
        // if(room.game.openDeck.length !=1 && room.game.openDeck[room.game.openDeck.length-1].length == 3){
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('PopOpenDeck',broObj);
				//Save History
				let dataObj = {
					playerId:currentPlayer.id,
    ...
    var currentPlayer = room.getCurrentPlayer();
        ...
        let i, j, tempi, tempj;
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('CloseDeckCardsRefill',{});
			}
			...
			await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('PopCloseDeck',broObj);
		//Save History
		let dataObj = {
			playerId:currentPlayer.id,
	...
	playerDrop: async function(data) {
		try{
     	 ...
     	 if (room.status != 'Running') {
			...
			await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('PlayerDrop', { 'playerId': data.playerId });

				roomUpdated = await Sys.Game.Practice.Deals.Services.RoomServices.update(room); // Update Player

				// room  =  await Sys.Game.Practice.Deals.Controllers.RoomProcess.broadcastPlayerInfo(room);
				room  =  await Sys.Game.Practice.Deals.Controllers.RoomProcess.checkTurnFinished(room,data.playerId);
					// Save History
					let dataObj = {
						playerId:currentPlayer.id,
						action: 'Player Drop',
  ...
  playerTableCards: async function(data) {
    ...
    if (room.status != 'Running') {
		...
		if (room.game.status == 'Running') {
			...
			await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('PushFinishDeck',broObj);
			roomUpdated = await Sys.Game.Practice.Deals.Services.RoomServices.update(room);
			...
			Sys.Timers[room.id] = setInterval(async function(room){
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('OnFinishTimer', {
					roomId: room.id,
					timer : timer,
					playerId : data.playerId,
	...
	declareGame: async function(data) {
		...
		if (room.game.status == 'Running') {
			...
			room = await Sys.Game.Practice.Deals.Controllers.RoomProcess.saveHistory(room,dataObj);
				...
				//console.log("currentPlayer.playerScore ++++++++++>",currentPlayer.playerScore);
					if(currentPlayer.playerScore == 0){
						...
						roomUpdated = await Sys.Game.Practice.Deals.Services.RoomServices.update(room); // Update room

						if(room.game.declare.length == 1){ // if Some one Already declare so not call declare
							room  =  await Sys.Game.Practice.Deals.Controllers.RoomProcess.waitForGameDeclare(room,data.playerId);
						}
           			 ...
           			 await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('CloseFinishTimer', {});
						currentPlayer.dropped = true;
            currentPlayer.playerScore = 80;
			...
			// Other Player Decalre
				...
				if(allDeclare){
					clearTimeout(Sys.Timers[room.id]); // Clear waitforDeclare
					room  =  await Sys.Game.Practice.Deals.Controllers.RoomProcess.gameFinished(room);
				}
    ...
    try {
  		...
  		Sys.Timers[room.id] = setInterval(async function(room){
  			...
  			await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(room.id).emit('OnDeclareTimer', {
  				roomId: room.id,
  				timer : timer,
  				playerId : playerId,
  ...
  //     //
	// 		// 			await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(socket.id).emit('PlayerInfo', playerInfoObj);
	// 		// 		}
	// 		// 	}
	// 		// }
  //     //
  ...
  //         isPrintedJoker  : isPrintedJoker
	...
	// 			await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(socket.id).emit('JokerOpenCardInfo', joObj);
  //
  //
	// 			room.players.forEach(async function (player) {
	// 				let playerCards = {
	...
	// 					cards : player.cards,
	// 				};
	// 				await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(socket.id).emit('ReconnectPlayerDeck', playerCards);
	// 			});
	// 			await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(socket.id).emit('TurnPlayer', {
	// 				playerId: room.getCurrentPlayer().id
	// 			});
	// 		}
  //
      ...
      // if(room.game && room.game.status == 'Running'){
        ...
        room.players.forEach(async function (player) {
          if (player.turnCount < 2 && player.id == data.playerId) {
            let	timer = parseInt(Sys.Config.Rummy.waitBeforeGameStart);
          await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(socket.id).emit('OnGameStartWait', {
            roomId: room.id,
            timer : timer,
            maxTimer :  parseInt(Sys.Config.Rummy.waitBeforeGameStart)
          });
        ...
        room.players.forEach(async function (player) {
          ...
          if(room.status=='Running' && player.id == data.playerId )
          {
            if ( player.turnCount < 2 ) {
              ...
              await Sys.Io.of(Sys.Config.Namespace.PracticeDeals).to(socket.id).emit('PlayerDeck', playerCards);
            } else {
