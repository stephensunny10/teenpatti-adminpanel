...
module.exports = {
    ...
    update: async function(room){
        ...
        try {
			if(tempRoom){
                if (tempRoom.game) {
                  let  updatedGame = await gameModel.findOneAndUpdate({_id: tempRoom.game.id},tempRoom.game, {new: true});
                  tempRoom.game = updatedGame.id; // Store Game ID for Save Room
                }
                let updatedtempRoom = await roomModel.findOneAndUpdate({_id: tempRoom.id},tempRoom, {new: true});
				return room;
			}else{
