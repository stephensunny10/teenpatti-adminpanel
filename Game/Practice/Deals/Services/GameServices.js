    ...
    create: async function(data){
       // //console.log('Create Game Called -------:',data)
        try {
			...
			let game =  new Sys.Game.Practice.Deals.Entities.Game(null,gameObj.roomId,gameObj.status, gameObj.pot, gameObj.history, gameObj.gameNumber, gameObj.gameType, gameObj.jokerCard, gameObj.openDeck, gameObj.closeDeck, gameObj.finishDeck, gameObj.declare);
            
            let gameSave = new gameModel(game);
            game = await gameSave.save(); // Save Game
            if(game){
                 
                                
                    return new Sys.Game.Practice.Deals.Entities.Game(game.id,game.roomId,game.status, game.pot, game.history, game.gameNumber, game.gameType, game.jokerCard, game.openDeck, game.closeDeck, game.finishDeck, game.declare); // return Room 
