...
class Room {
  constructor(id, tableType, name,tableNumber,dealer, minPlayers, maxPlayers, type, owner,gameType, rack, pointsValue, entryFees, currentPlayer, players, gameWinners, gameLosers,status,  game,varient, numberOfDecks ,  printedJoker,timerStart,gameOverPoint, upperJoker, lowerJoker, valueCards) { // TWENTYONE_CHANGE
    var room                  = this;
    this.id                   = id;
    ...
    if (players && Array.isArray(players)) {
        ...
        players.forEach(function (player) {
          room.players.push(new Sys.Game.Practice.PointsTwentyOne.Entities.Player().createObject(player));
        })
    }
    this.gameWinners          = [];
    ...
    this.game                 = null
    if (game) {
      this.game = new Sys.Game.Practice.PointsTwentyOne.Entities.Game().createObject(game);
    }
  ...
  toJson() {
    ...
    // other field
    if (this.game) {
      //room.game = new Sys.Game.Practice.PointsTwentyOne.Entities.Game().createObject(this.game);
      room.game = this.game.toJson();
      // new Sys.Game.Practice.PointsTwentyOne.Entities.Game(this.game.id,this.game.roomId,this.game.status, this.game.pot, this.game.history, this.game.gameNumber, this.game.gameType, this.game.jokerCard, this.game.openDeck, this.game.closeDeck, this.game.finishDeck, this.game.declare);
    }
    return room
  }
  ...
  async AddPlayer(id, socketId, username, status, avatar, appId, chips,cash, seatIndex, isBot) {
    ...
    let plyObj = {
      ...
      pointValue        : 80,
      totalPoint        : 0,
      playerScore       : 120,                                              // TWENTYONE_CHANGE
      cardsString       : '-',
      seatIndex         : seatIndex,
      isBot             : isBot,
      cardTurn          : true,
    ...
    let player = new Sys.Game.Practice.PointsTwentyOne.Entities.Player().createObject(plyObj);

    //  let player = new Sys.Game.Practice.PointsTwentyOne.Entities.Player(id,socketId,username,avatar,status,appId,chips,cash,80,0,0,'-',seatIndex,isBot,true,false,false,0,1,[],0,Sys.Config.Rummy.extraTime,0);
  ...
  async StartGame() {
    ...
    if (!room.game && playersLength >= this.minPlayers) {
      ...
      let game = await Sys.Game.Practice.PointsTwentyOne.Services.GameServices.create(gameobj);
      ...
      if (game) {
        ...
        if (playersLength >= room.minPlayers) {
          ...
          let gameStarted = await Sys.Game.Practice.PointsTwentyOne.Controllers.RoomProcess.newGameStarted(room);
          room.initNewRound();
        }
      }
    }
  ...
  async initNewRound() {
    ...
    let deck =  new Sys.Game.Practice.PointsTwentyOne.Entities.Deck();
    ...
    // // Get Joker Card
     ...
     // room.game.openDeck =  joker; // Remove the item from the array
    ...
    //console.log("88888888888888888888888888888888888888888888888888888888888888");
    //console.log("joker[0] ->",joker[0]);
    //console.log("upperJoker[0] ->",upperJoker[0]);
    //console.log("lowerJoker[0] ->",lowerJoker[0]);
    //console.log("88888888888888888888888888888888888888888888888888888888888888");
    //console.log("88888888888888888888888888888888888888888888888888888888888888");
   ...
   // room  =  await Sys.Game.Practice.PointsTwentyOne.Controllers.RoomProcess.broadcastPlayerInfo(room);
    ...
    let priorityCards = await Sys.Game.Practice.PointsTwentyOne.Controllers.RoomProcess.turnPlayerSelection(room,cardsArray);
    //console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
...
//console.log("Player List ->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.",playerInfoDummy)
      await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyOne).to(room.id).emit('PlayerList', playerInfoDummy, { isBeforeGameStart: roomStatus });
  ...
  async NewRound() {
    ...
    for (i = 0; i < room.players.length; i += 1) {
      if(room.players[i].status == 'playing'){
        ...
        room.players[i].cards.push(room.game.closeDeck.pop());
        room.players[i].cards.push(room.game.closeDeck.pop());
        // room.players[i].cards.push('JH');
        // room.players[i].cards.push('KHJ');
        // room.players[i].cards.push('QHJ');
        // room.players[i].cards.push('AHJ');
      }
    ...
    room.status = 'Running';
    room = await Sys.Game.Practice.PointsTwentyOne.Services.RoomServices.update(room);
    let newRoundStarted = await Sys.Game.Practice.PointsTwentyOne.Controllers.RoomProcess.newRoundStarted(room);
