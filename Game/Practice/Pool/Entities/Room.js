...
class Room {
  constructor(id, tableType, name,tableNumber,dealer, minPlayers, maxPlayers, type, owner,gameType,rack,entryFees, currentPlayer, players, gameWinners, gameLosers,status,  game,varient, numberOfDecks ,  printedJoker,timerStart,gameOverPoint,gamecount) {
    var room = this;
    this.id = id;
    ...
    //if (gameLosers) {
      ...
      //   player.room = room
      //   room.gameLosers.push(load('Games/Practice/Points/Entities/Player', player))
      // })
    //}
    this.game = null
    if (game) {
     this.game = new Sys.Game.Practice.Pool.Entities.Game(game.id,game.roomId,game.status, game.pot, game.history, game.gameNumber, game.gameType, game.jokerCard, game.openDeck, game.closeDeck, game.finishDeck, game.declare);
    }
  ...
  toJson() {
    ...
    // other field
    if (this.game) {
      room.game = new Sys.Game.Practice.Pool.Entities.Game(this.game.id,this.game.roomId,this.game.status, this.game.pot, this.game.history, this.game.gameNumber, this.game.gameType, this.game.jokerCard, this.game.openDeck, this.game.closeDeck, this.game.finishDeck, this.game.declare);
    }
    return room
  }
  ...
  async AddPlayer(id, socketId, username, status, avatar, appId, chips,cash, seatIndex, isBot) {
    ...
    let room = this
    let player = new Sys.Game.Practice.Pool.Entities.Player(id,socketId,username,avatar,status,appId,chips,cash,80,0,80,'-',seatIndex,isBot,true,false,false,0,1,[],0,Sys.Config.Rummy.extraTime,1);
  ...
  async StartGame() {
    ...
    if (!room.game && playersLength >= this.minPlayers) {
      ...
      let game = await Sys.Game.Practice.Pool.Services.GameServices.create(gameobj);
      ...
      if (game) {
        ...
        if (playersLength >= room.minPlayers) {
          ...
          room.status = 'Running';
          // Sys.Game.Practice.Pool.Services.RoomServices.update(room);
          //console.log("Game Starting.......");
          let gameStarted = await Sys.Game.Practice.Pool.Controllers.RoomProcess.newGameStarted(room);
          room.initNewRound();
        }
      }
    }
  ...
  async initNewRound() {
    ...
    let deck =  new Sys.Game.Practice.Pool.Entities.Deck();
...
////console.log("Joker ->",joker);
    ...
    //console.log("================================================");
    if (room.gamecount == 1) {
      let priorityCards = await Sys.Game.Practice.Pool.Controllers.RoomProcess.turnPlayerSelection(room,cardsArray);
    ...
    let playerInfoDummy = [];
     ...
     // //console.log("Player List ->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.",playerInfoDummy)
      await Sys.Io.of(Sys.Config.Namespace.PracticePool).to(room.id).emit('PlayerList', playerInfoDummy, { isBeforeGameStart: roomStatus });
  ...
  async NewRound() {
    ...
    room.status = 'Running';
    room = await Sys.Game.Practice.Pool.Services.RoomServices.update(room);
    let newRoundStarted = await Sys.Game.Practice.Pool.Controllers.RoomProcess.newRoundStarted(room);
