...
class Room {
  constructor(id, tableType, name,tableNumber,dealer, minPlayers, maxPlayers, type, owner,gameType, rack, pointsValue, entryFees, currentPlayer, players, gameWinners, gameLosers,status,  game,varient, numberOfDecks ,  printedJoker,timerStart,gameOverPoint, upperJoker, lowerJoker, valueCards, superLowerJoker, superUpperJoker) { // TWENTYONE_CHANGE
    ...
    if (players && Array.isArray(players)) {
        ...
        players.forEach(function (player) {
          room.players.push(new Sys.Game.Practice.PointsTwentyFour.Entities.Player().createObject(player));
        })
    }
    this.gameWinners          = [];
    ...
    this.game                 = null
    if (game) {
      this.game               = new Sys.Game.Practice.PointsTwentyFour.Entities.Game().createObject(game);
    }
  ...
  toJson() {
    ...
    // other field
    if (this.game) {
      //room.game = new Sys.Game.Practice.PointsTwentyFour.Entities.Game().createObject(this.game);
      room.game = this.game.toJson();
      // new Sys.Game.Practice.PointsTwentyFour.Entities.Game(this.game.id,this.game.roomId,this.game.status, this.game.pot, this.game.history, this.game.gameNumber, this.game.gameType, this.game.jokerCard, this.game.openDeck, this.game.closeDeck, this.game.finishDeck, this.game.declare);
    }
    return room
  }
  ...
  async AddPlayer(id, socketId, username, status, avatar, appId, chips,cash, seatIndex, isBot) {
    ...
    let plyObj = {
      ...
      pointValue        : 80,
      totalPoint        : 0,
      playerScore       : 130,                                              // TWENTYONE_CHANGE
      cardsString       : '-',
      seatIndex         : seatIndex,
      isBot             : isBot,
      cardTurn          : true,
    ...
    let player = new Sys.Game.Practice.PointsTwentyFour.Entities.Player().createObject(plyObj);

    //  let player = new Sys.Game.Practice.PointsTwentyFour.Entities.Player(id,socketId,username,avatar,status,appId,chips,cash,80,0,0,'-',seatIndex,isBot,true,false,false,0,1,[],0,Sys.Config.Rummy.extraTime,0);
  ...
  async StartGame() {
    ...
    if (!room.game && playersLength >= this.minPlayers) {
      ...
      let game = await Sys.Game.Practice.PointsTwentyFour.Services.GameServices.create(gameobj);
      ...
      if (game) {
        ...
        if (playersLength >= room.minPlayers) {
          ...
          let gameStarted = await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.newGameStarted(room);
          room.initNewRound();
        }
      }
    }
  ...
  async initNewRound() {
    ...
    let deck =  new Sys.Game.Practice.PointsTwentyFour.Entities.Deck();
    ...
    //console.log("Joker ->",joker);
    //console.log("room.game.superLowerJoker ->",room.game.superLowerJoker);
    //console.log("room.game.superUpperJoker ->",room.game.superUpperJoker);
    // // End Here
   ...
   // room  =  await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.broadcastPlayerInfo(room);
    ...
    let priorityCards = await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.turnPlayerSelection(room,cardsArray);
    //console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
...
//console.log("Player List ->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.",playerInfoDummy)
      await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('PlayerList', playerInfoDummy, { isBeforeGameStart: roomStatus });
  ...
  async NewRound() {
    ...
    room.status = 'Running';
    room = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.update(room);
    let newRoundStarted = await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.newRoundStarted(room);
