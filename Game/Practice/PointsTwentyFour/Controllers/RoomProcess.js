...
module.exports = {
      ...
      try {
        ...
        rooms = await Sys.Game.Common.Services.RoomServices.getByData(query);
          ...
          if (isEmptySpace == false) {
              ...
              let rm = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.create(data);
              if (!rm) {
                  return { status: 'fail', result: null, message: 'No Room Created 1.', statusCode: 401 }
              }
              room = rm; // Assign to Room
		}
		room = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.get(room.id); //// Just Get Table Data With Format.
  ...
  joinRoom: async function(player,data){
      try {
		...
		let room = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.get(data.roomId);
        ...
        if (oldPlayer.cards.length > 24) {
          oldPlayer.cardTurn  = true;
				...
				roomUpdated = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.update(room);
        ...
        await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('JoinedPlayerInfo',joinedPlayer, { isBeforeGameStart: roomStatus });
        await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(data.socketId).emit('PlayerList', playerInfoDummy, { isBeforeGameStart: roomStatus });
				Sys.Log.info('<=> Player count (NewPlayer) || Player Length :'+totalPlayers);
				if (totalPlayers > 0) {

					// if (totalPlayers > 0) {
					// 	// room  =  await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.broadcastPlayerInfo(room);
					// }
					...
					if (room.status != 'Running' && playersLength >= room.minPlayers) {
						...
						if (room.game == null && room.timerStart == false) {
							...
							room = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.update(room);
							...
							let	timer = parseInt(Sys.Config.Rummy.waitBeforeGameStart);
								Sys.Timers[room.id] = setInterval(async function(room){
									//console.log("OnGameStartWait Send => ",Sys.Config.Namespace.PracticePointsTwentyFour,'---', room.id);

									await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('OnGameStartWait', {
										roomId: room.id,
										timer : timer,
                       ...
                       room.status = 'Running';
                       await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.update(room);
											room.StartGame();
										 }else{
  ...
  newGameStarted: async function(room) {
  	try {
  		//console.log('<=>Practice Game Started Brodcast || GameStarted :',Sys.Config.Namespace.PracticePointsTwentyFour,room.id);
  		await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('GameStarted', {
	...
	turnPlayerSelection: async function(room,cardsArray) {
		try{
			...
			await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('priorityCards', sellectObj);
			return room;
		}catch (error) {
	...
	newRoundStarted: async function(room) {
      ...
      if (room.game.jokerCard[0] == 'PJ') {
        ...
        isPrintedJoker  : isPrintedJoker
			...
			await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('JokerOpenCardInfo', joObj);
      // //console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
      // //console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
      // //console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
      ...
      // //console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
			...
			room.players.forEach(async function (player) {
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('PlayerDeck', playerCards);
			});
			...
			Sys.Timers[room.id] = setTimeout(async function (room) {
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('TurnPlayer', {
					playerId: room.players[room.currentPlayer].id
				});

					let timer = parseInt(Sys.Config.Rummy.turnTime24);
					Sys.Timers[room.id] = setInterval(async function(room){
						//console.log("turnTime First Timer Send ->",timer);
						await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('OnTurnTimer', {
							playerId: room.players[room.currentPlayer].id,
							timer : timer,
							maxTimer :  parseInt(Sys.Config.Rummy.turnTime24),
							name : 'turnTime'
						});
						timer--;
						if(timer < 1){
							...
							if(extraTimer < 1){
								...
								Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.playerDefaultAction(room.id);
							}else{
								Sys.Timers[room.id] = setInterval(async function(room){
	...
	playerDefaultAction: async function(id) {
		...
		let room = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.get(id);
        ...
        currentPlayer.dropped = true;
				...
				let responce =  await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.leftRoom(playerData);
        if(responce instanceof Error){
				...
				room = await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.saveHistory(room,dataObj);
				...
				if(playercards == 26){
					...
					await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('OnAutoDiscard', {
						playerId: currentPlayer.id,
						card : card[0],
					});
				...
				roomUpdated = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.update(room);
				await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.changeTurn(room);
				await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.turnFinished(room);
			}
		  ...
		  ////console.log('<=> Removing Player Room Not Found');
		   ...
		   if (room.players.length > 0) {
             ...
             // Push Player in Looser Array.
              ...
              let playerObj = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.getById(room.players[i].id);
              ...
              let updatedPlayer = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.update(playerObj.id,{ chips: newChips });
		   ...
		   if (player) {
			  // //console.log('<=> Removing Player || GAME-NUMBER [] ||');
			   ...
			   if (room.game && room.game.status == 'Running') {
				   ...
				   room  = await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.checkTurnFinished(room,playerId);
		   ...
		   await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('PlayerLeft', { 'playerId': player.id });
		...
		try {
			...
			for (var i = 0; i < room.players.length; i++) {
				if(room.players[i].status != 'Left'){
					...
					// await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('PlayerInfo', playerInfoObj);
				}
			}
    for (var i = 0; i < 6; i++) {
      let seatMatch = false;
    ...
    // dataArr.push({vacantSeat: playerFreeSeats});
		await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('playerInfoDummy', playerInfoDummy,{vacantSeat: playerFreeSeats});
		return room;
	 }catch (error) {
	...
	checkTurnFinished: async function(room,playerId) {
		...
		if (room.getCurrentPlayer().id == playerId) {
			...
			if(playinPlayer == 1){
				...
				await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.gameFinished(room);
  						...
  						if( timer < 1){
  							...
  							Sys.Timers[room.id] =  setInterval(async function(room){
  								...
  								if(timer < 1){
  										clearTimeout(Sys.Timers[room.id]); // Clear Turn Timer
  										//console.log("Turn - check is timer not Close...");
  										await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.playerDefaultAction(room.id);
	...
	gameFinished : async function(room) {
      ...
      // }
  		...
  		if(playinPlayer != 1){
            ...
            // winnerPlayers[0].won = (parseInt(winnerPlayers[0].won) + ( parseInt(LosserPlayer.playerScore) * parseInt(room.pointsValue))).toString(); // Push Winner Amount.
  					...
  					// let playerObj = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.getById(LosserPlayer.id);
            ...
            //   chips             : parseInt(2 * parseInt(room.pointsValue)),
  					...
  					// 	status           : 'sucess',
  					// }
  					// let chipsTransection = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.cerateChipsCashHistory(trnsObj);
          ...
          //   let updatedAdminCommission = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.createCommission({
          //     roomid                : room.id,
        ...
        // }
  			...
  			//winnerPlayers[0].won
  			// let playerObj = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.getById(winnerPlayers[0].playerId);
  			// let newChips = parseInt(playerObj.chips)+parseInt(winnerPlayers[0].won);
  			// let updatedPlayer = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.update(winnerPlayers[0].playerId,{ chips: newChips });
        ...
        if ( parseInt(winner.valuecards) > 0) {
          ...
          wrongfinished   : winner.wrongfinished
  			...
  			// let playerObj = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.getById(winner.id);
  			// let newChips = parseInt(playerObj.chips)+parseInt(singlePlayerWinnigchips) ;
  			// let updatedPlayer = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.update(winner.id,{ chips: newChips });
      ...
      for (var k = 0; k < winnerPlayers.length; k++) {
        if (winnerPlayers[k].type == 'winner') {
          ...
          // //console.log("Sys.Config.Rummy.cardScore_24_+''+room.players[i].jokercardnumber", Sys.Config.Rummy['cardScore_24_'+LosserPlayer.jokercardnumber]);
          // //console.log("Sys.Config.Rummy.cardScore_24_+''+room.players[i].uppercardnumber", Sys.Config.Rummy['cardScore_24_'+LosserPlayer.uppercardnumber]);
          // //console.log("Sys.Config.Rummy.cardScore_24_+''+room.players[i].lowercardnumber", Sys.Config.Rummy['cardScore_24_'+LosserPlayer.lowercardnumber]);
          // //console.log("Sys.Config.Rummy.marrigeScore_24_+''+room.players[i].marriageindex", Sys.Config.Rummy['cardScore_24_'+LosserPlayer.marriageindex]);
          // Credit winner Chips
          let playerObj = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.getById(winnerPlayers[k].playerId);
          // TWENTYONE_CHANGE
          ...
          //console.log("parseFloat(playerObj.chips)", parseFloat(playerObj.chips));
          //console.log("parseFloat(winnerPlayers[k].valueCardScore.won)", parseFloat(winnerPlayers[k].won));
          ...
          if (LosserPlayer && LosserPlayer.status != 'Left') {
            newChips = parseFloat(playerObj.chips) + parseFloat(winnerPlayers[k].won);
            if (parseFloat(winnerPlayers[k].won) > playerObj.chips) {
              newChips = 0;
            }
            let updatedPlayer = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.update(playerObj.id,{ chips: newChips });
          }
          ...
          let trnsObj = {
          	playerId          : playerObj.id,
            gameId            : room.game.id,
            chips             : (parseFloat(winnerPlayers[k].won) * (-1)),
          	// chips             : parseInt(room.players[i].playerScore) * parseInt(Sys.Config.Rummy.pointsValue),
          	cash              : 0,
          	type              : 'debit',   //debit/credit
          	tranType 			    : 'chips',
  		...
  		Sys.Timers[room.id] = setInterval(async function(room){
  			//console.log("Practice Points24 Fainal Game Finsihed Timer ",timer)
        ...
        // }
  			...
  			await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('OnGameFinished', {
  			...
  			if(timer < 1){
  				...
  				await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('OnGameRestart', {});
  				for (i = 0; i < room.players.length; i++) {
  					if(room.players[i].status != 'Left'){
                ...
                for (var i = 0; i < room.players.length; i++) {
                  let player = await Sys.Game.Practice.PointsTwentyFour.Services.PlayerServices.getById(room.players[i].id);
                  //console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
                  ...
                  if (parseFloat(player.chips) < parseFloat( parseFloat( parseFloat(room.pointsValue)*130 ) + 700 )) {
                    ...
                    await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.leaveRoom(playerData);
            				// await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.leaveRoom(playerData);
	...
	pushOpenDeck: async function(data) {
		try{
            ...
            break;
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('PushOpenDeck',broObj);
	...
	popOpenDeck: async function(data) {
        ...
        // if(room.game.openDeck.length !=1 && room.game.openDeck[room.game.openDeck.length-1].length == 3){
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('PopOpenDeck',broObj);
				//Save History
				let dataObj = {
					playerId:currentPlayer.id,
	...
	popCloseDeck: async function(data) {
		...
		//console.log("room.game.status->",room.game.status);
		 // await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('CloseDeckCardsRefill',{});
		...
		if (room.game.status == 'Running') {
			...
			await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('PopCloseDeck',broObj);
	...
	playerDrop: async function(data) {
		try{
			...
			await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('PlayerDrop', { 'playerId': data.playerId });

				roomUpdated = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.update(room); // Update Player

				//room  =  await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.broadcastPlayerInfo(room);
				room  =  await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.checkTurnFinished(room,data.playerId);
	...
	declarefinishGame: async function(data) {
		...
		if (room.game.status == 'Running') {
          ...
          break;
			...
			await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('PushFinishDeck',broObj);
			roomUpdated = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.update(room);
			...
			Sys.Timers[room.id] = setInterval(async function(room){
				...
				await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('OnFinishTimer', {
					roomId: room.id,
					timer : timer,
					playerId : data.playerId,
				...
				if(timer == 0){
					...
					if (room.players.length > 0) {
						for (let i = 0; i < room.players.length; i++) {
							if (room.players[i].id == data.playerId && room.players[i].status != 'Left') {
								room.players[i].dropped = true;
								// room.players[i].playerScore =  130;                             // TWENTYONE_CHANGE
	...
	declareGame: async function(data) {
		...
		if (room.game.status == 'Running') {
			...
			room = await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.saveHistory(room,dataObj);
				...
				//console.log("currentPlayer.playerScore ++++++++++>",currentPlayer.playerScore);
					if(currentPlayer.playerScore == 0){
						...
						roomUpdated = await Sys.Game.Practice.PointsTwentyFour.Services.RoomServices.update(room); // Update room

						if(room.game.declare.length == 1){ // if Some one Already declare so not call declare
							room  =  await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.waitForGameDeclare(room,data.playerId);
						}
						...
						await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('CloseFinishTimer', {});
						currentPlayer.dropped = true;
			...
			// Other Player Decalre
				...
				if(allDeclare){
					clearTimeout(Sys.Timers[room.id]); // Clear waitforDeclare
					room  =  await Sys.Game.Practice.PointsTwentyFour.Controllers.RoomProcess.gameFinished(room);
				}
	...
	waitForGameDeclare: async function(room,playerId) {
		try {
  		...
  		Sys.Timers[room.id] = setInterval(async function(room){
  			...
  			await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(room.id).emit('OnDeclareTimer', {
  				roomId: room.id,
  				timer : timer,
  				playerId : playerId,
  ...
  // 				jokerCard   : room.game.jokerCard[0],
	...
	// 			await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(socket.id).emit('JokerOpenCardInfo', joObj);
  //
  //
	// 			room.players.forEach(async function (player) {
	// 				let playerCards = {
	...
	// 					cards : player.cards,
	// 				};
	// 				await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(socket.id).emit('ReconnectPlayerDeck', playerCards);
	// 			});
	// 			await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(socket.id).emit('TurnPlayer', {
	// 				playerId: room.getCurrentPlayer().id
	// 			});
	// 		}
  //
      ...
      // if(room.game && room.game.status == 'Running'){
        ...
        room.players.forEach(async function (player) {
          if (player.turnCount < 2 && player.id == data.playerId) {
            let	timer = parseInt(Sys.Config.Rummy.waitBeforeGameStart);
          await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(socket.id).emit('OnGameStartWait', {
            roomId: room.id,
            timer : timer,
            maxTimer :  parseInt(Sys.Config.Rummy.waitBeforeGameStart)
          });
        ...
        room.players.forEach(async function (player) {
          ...
          if(room.status=='Running' && player.id == data.playerId )
          {
            if ( player.turnCount < 2 ) {
              ...
              await Sys.Io.of(Sys.Config.Namespace.PracticePointsTwentyFour).to(socket.id).emit('PlayerDeck', playerCards);
            } else {
