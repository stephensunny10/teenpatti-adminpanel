...
module.exports = {
    ...
    create: async function(data){
        ...
        try {
            ...
            // room.id = room._id
            if(room){
                Sys.Rooms[room.id] = new Sys.Game.CashGame.Points.Entities.Room().createObject(room);
    ...
    getCommission : async function(data){
        try {
          let value=await comissionModel.find(data);
          return value;
        } catch (error) {
         // Sys.Log.info('Error in createCommission  : ' + error);
          return new Error("Error in Update Room !");
