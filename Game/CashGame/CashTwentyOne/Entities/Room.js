...
class Room {
  constructor(id, tableType, name,tableNumber,dealer, minPlayers, maxPlayers, type, owner,gameType, rack, pointsValue, entryFees, currentPlayer, players, gameWinners, gameLosers,status,  game,varient, numberOfDecks ,  printedJoker,timerStart,gameOverPoint, upperJoker, lowerJoker, valueCards) {    // TWENTYONE_CHANGE
    ...
    if (players && Array.isArray(players)) {
        ...
        players.forEach(function (player) {
          room.players.push(new Sys.Game.CashGame.CashTwentyOne.Entities.Player().createObject(player));
        })
    }
    this.gameWinners          = [];
    ...
    this.game                 = null
    if (game) {
      this.game               = new Sys.Game.CashGame.CashTwentyOne.Entities.Game().createObject(game);
    }
  ...
  toJson() {
    ...
    // other field
    if (this.game) {
      //room.game = new Sys.Game.CashGame.CashTwentyOne.Entities.Game().createObject(this.game);
      room.game = this.game.toJson();
      // new Sys.Game.CashGame.CashTwentyOne.Entities.Game(this.game.id,this.game.roomId,this.game.status, this.game.pot, this.game.history, this.game.gameNumber, this.game.gameType, this.game.jokerCard, this.game.openDeck, this.game.closeDeck, this.game.finishDeck, this.game.declare);
    }
    return room
  }
  ...
  async AddPlayer(id, socketId, username, status, avatar, appId, chips,cash, seatIndex, isBot) {
    ...
    let player = new Sys.Game.CashGame.CashTwentyOne.Entities.Player().createObject(plyObj);

    //  let player = new Sys.Game.CashGame.CashTwentyOne.Entities.Player(id,socketId,username,avatar,status,appId,chips,cash,80,0,0,'-',seatIndex,isBot,true,false,false,0,1,[],0,Sys.Config.Rummy.extraTime,0);
  ...
  async StartGame() {
    ...
    if (!room.game && playersLength >= this.minPlayers) {
      ...
      let game = await Sys.Game.CashGame.CashTwentyOne.Services.GameServices.create(gameobj);
      ...
      if (game) {
        ...
        if (playersLength >= room.minPlayers) {
          ...
          let gameStarted = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.newGameStarted(room);
          room.initNewRound();
        }
      }
    }
  ...
  async initNewRound() {
    ...
    let deck =  new Sys.Game.CashGame.CashTwentyOne.Entities.Deck();
    ...
    //console.log("//////////////////////////////////////////");
    //console.log("//////////////////////////////////////////");
    //console.log("room.game.jokerCard", room.game.jokerCard);
    //console.log("room.printedJoker", room.printedJoker);
    //console.log("room.game.upperJoker", room.game.upperJoker);
    //console.log("room.upperJoker", room.upperJoker);
    //console.log("room.game.lowerJoker", room.game.lowerJoker);
    //console.log("room.lowerJoker", room.lowerJoker);
    //console.log("//////////////////////////////////////////");
    //console.log("//////////////////////////////////////////");
    //console.log("//////////////////////////////////////////");
    //console.log("//////////////////////////////////////////");
   ...
   // room  =  await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.broadcastPlayerInfo(room);
    ...
    let priorityCards = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.turnPlayerSelection(room,cardsArray);
    //console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
...
//console.log("Player List ->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.",playerInfoDummy)
      await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyOne).to(room.id).emit('PlayerList', playerInfoDummy, { isBeforeGameStart: roomStatus });
  ...
  async NewRound() {
    ...
    room.status = 'Running';
    room = await Sys.Game.CashGame.CashTwentyOne.Services.RoomServices.update(room);
    let newRoundStarted = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.newRoundStarted(room);
