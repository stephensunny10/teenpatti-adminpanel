...
module.exports = {
	...
	joinRoom: async function (socket, data) {
		try {
			let player = await Sys.Game.CashGame.CashTwentyOne.Services.PlayerServices.getById(data.playerId);
			if (!player) {
			...
			let room = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.checkRoomSeatAvilability(data);
			...
			room = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.joinRoom(player, data);
			if (room instanceof Error) {
				return { status: 'fail', result: null, message: room.message, statusCode: 401 }
			}
			...
			socket.myData.gameType = 'cash_points21';
	...
	pushOpenDeck: async function (socket, data) {
		try {
			let action = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.pushOpenDeck(data);
	...
	popOpenDeck: async function (socket, data) {
		try {
			let action = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.popOpenDeck(data);
			if (action instanceof Error) {
				return { status: 'fail', result: null, message: action.message, statusCode: 401 }
			}
			return {
	...
	popCloseDeck: async function (socket, data) {
		try {
			let action = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.popCloseDeck(data);
			if (action instanceof Error) {
				return { status: 'fail', result: null, message: action.message, statusCode: 401 }
			}
			return {
	...
	leaveRoom: async function (socket, data) {
		try {
			let responce = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.leaveRoom(data);
	...
	getOpenDeckList: async function (socket, data) {
		try {
			let action = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.getOpenDeckList(data);
			if (action instanceof Error) {
				return { status: 'fail', result: null, message: action.message, statusCode: 401 }
			}
			return {
	...
	playerDrop: async function (socket, data) {
		try {
			let action = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.playerDrop(data);
			if (action instanceof Error) {
				return { status: 'fail', result: null, message: action.message, statusCode: 401 }
			}
			return {
	...
	playerCardsScore: async function (socket, data) {
		try {
			let action = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.playerCardsScore(data);
			if (action instanceof Error) {
				return { status: 'fail', result: null, message: action.message, statusCode: 401 }
			}
			return {
	...
	playerTableCards: async function (socket, data) {
		try {
			let action = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.playerTableCards(data);
			if (action instanceof Error) {
				return { status: 'fail', result: null, message: action.message, statusCode: 401 }
			}
			return {
	...
	playerCards: async function (socket, data) {
		try {
			...
			let action = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.playerCards(data);
			if (action instanceof Error) {
				return { status: 'fail', result: null, message: action.message, statusCode: 401 }
			}
			//console.log("##############################################");
	...
	declarefinishGame: async function (socket, data) {
		try {
			let action = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.declarefinishGame(data);
			if (action instanceof Error) {
				return { status: 'fail', result: null, message: action.message, statusCode: 401 }
			}
			return {
	...
	declareGame: async function (socket, data) {
		try {
			let action = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.declareGame(data);
			if (action instanceof Error) {
				return { status: 'fail', result: null, message: action.message, statusCode: 401 }
			}
			return {
	...
	reconnectGame: async function (socket, data) {
		try {
			...
			let room = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.findGame(data);
			if (room instanceof Error) {
				return { status: 'fail', result: null, message: room.message, statusCode: 401 }
			}
			socket.join(room.id); // Subscribe Room.

			room = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.reconnectGame(socket, data);
			if (room instanceof Error) {
				return { status: 'fail', result: null, message: room.message, statusCode: 401 }
			}
	...
	disconnectPlayer: async function (socket, data) {
		try {
			...
			if (!room) {
				...
				// let responce = await Sys.Game.CashGame.CashTwentyOne.Controllers.RoomProcess.leaveRoom(leaveRoomData);
				// if (responce instanceof Error) {
				// 	return { status: 'fail', result: null, message: responce.message, statusCode: 401 }
				// }
				// if (responce.status == 'success') {
