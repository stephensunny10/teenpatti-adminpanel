...
module.exports = {
      ...
      try {
        ...
        rooms = await Sys.Game.Common.Services.RoomServices.getByData(query);
          ...
          if (isEmptySpace == false) {
              ...
              let rm = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.create(data);
              if (!rm) {
                  return { status: 'fail', result: null, message: 'No Room Created 1.', statusCode: 401 }
              }
              room = rm; // Assign to Room
		}
		room = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.get(room.id); //// Just Get Table Data With Format.
  ...
  joinRoom: async function(player,data){
      try {
		...
		let room = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.get(data.roomId);
		...
		if (seatAvailable) {
			...
			if (oldPlayer)
			{
				...
				oldPlayer.playerScore = 130;
				oldPlayer.totalPoint  = 0;
				oldPlayer.turnCount   = 1;

        oldPlayer.cardTurn    = false;
        if (oldPlayer.cards.length > 24) {
          oldPlayer.cardTurn  = true;
        }
				roomUpdated = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.update(room);
        ...
        await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('JoinedPlayerInfo',joinedPlayer, { isBeforeGameStart: roomStatus });
        await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(data.socketId).emit('PlayerList', playerInfoDummy, { isBeforeGameStart: roomStatus });
				...
				if (totalPlayers > 0) {
					...
					if (room.status != 'Running' && playersLength >= room.minPlayers) {
						...
						if (room.game == null && room.timerStart == false) {
							...
							let	timer = parseFloat(Sys.Config.Rummy.waitBeforeGameStart);
								Sys.Timers[room.id] = setInterval(async function(room){
									//console.log("OnGameStartWait Send => ",Sys.Config.Namespace.CashPointsTwentyFour,'---', room.id);

									await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('OnGameStartWait', {
										roomId: room.id,
										timer : timer,
                       ...
                       room.status = 'Running';
                       await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.update(room);
											room.StartGame();
										 }else{
  ...
  newGameStarted: async function(room) {
  	try {
  		//console.log('<=>CashGame Game Started Brodcast || GameStarted :',Sys.Config.Namespace.CashPointsTwentyFour,room.id);
  		await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('GameStarted', {
	...
	turnPlayerSelection: async function(room,cardsArray) {
      ...
      room.participantPlayers = [];
			...
			await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('priorityCards', sellectObj);
			return room;
		}catch (error) {
	...
	newRoundStarted: async function(room) {
      ...
      if (room.game.jokerCard[0] == 'PJ') {
        ...
        jokerCard       = 'AS';
			...
			let joObj = {
        upperJoker2     : room.game.superUpperJoker[0],
        upperJoker1     : room.game.upperJoker[0],
        lowerJoker2     : room.game.superLowerJoker[0],
        lowerJoker1     : room.game.lowerJoker[0],
				jokerCard       : jokerCard,
				OpenCard        : room.game.openDeck[0],
        isPrintedJoker  : isPrintedJoker
			}
			await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('JokerOpenCardInfo', joObj);
			...
			room.players.forEach(async function (player) {
				...
				await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('PlayerDeck', playerCards);
			});
			...
			Sys.Timers[room.id] = setTimeout(async function (room) {
				...
				await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('TurnPlayer', {
					playerId: room.players[room.currentPlayer].id
				});

					let timer = parseFloat(Sys.Config.Rummy.turnTime24);
					Sys.Timers[room.id] = setInterval(async function(room){
						//console.log("turnTime First Timer Send ->",timer);
						await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('OnTurnTimer', {
							playerId: room.players[room.currentPlayer].id,
							timer : timer,
							maxTimer :  parseFloat(Sys.Config.Rummy.turnTime24),
							name : 'turnTime'
						});
						timer--;
						if(timer < 1){
							...
							if(extraTimer < 1){
								...
								Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.playerDefaultAction(room.id);
							}else{
								Sys.Timers[room.id] = setInterval(async function(room){
	...
	playerDefaultAction: async function(id) {
		...
		let room = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.get(id);
        ...
        currentPlayer.dropped = true;
				...
				let responce =  await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.leftRoom(playerData);
        if(responce instanceof Error){
				...
				room = await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.saveHistory(room,dataObj);
				...
				if(playercards == 25){
					...
					await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('OnAutoDiscard', {
						playerId: currentPlayer.id,
						card : card[0],
					});
				...
				roomUpdated = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.update(room);
				await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.changeTurn(room);
				await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.turnFinished(room);
			}
	...
	leftRoom: async function(data) {
		try {
		  ...
		  ////console.log('<=> Removing Player Room Not Found');
		   ...
		   if (room.players.length > 0) {
             ...
             if (room.players[i].dropped == false) {
              if(room.players[i].turnCount == 2  || room.players[i].turnCount == 1 ){
  						  room.players[i].playerScore = Sys.Config.Rummy.playerFirstDrop_24;
  						}else if(room.players[i].turnCount == 3){
  							room.players[i].playerScore = Sys.Config.Rummy.playerSecondDrop_24;
  						}else{
  							room.players[i].playerScore = Sys.Config.Rummy.playerThirdDrop_24;
  						}
             }
             ...
             // Push Player in Looser Array.
              ...
              let playerObj = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.getById(room.players[i].id);
              ...
              let playerCashManager = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.getCashManager({ playerId: playerObj.id });
              ...
              let updatedPlayerCash = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.updateCashManager({ playerId: playerObj.id },cashManageData);

              let updatedPlayer = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.update(playerObj.id,{ cash: newTotalCash, rewardPoint: playerCashManager.rewardPoint + winningRewardPoint, cashTransaction:(parseFloat(playerObj.cashTransaction) + parseFloat(wonValue*(-1)) )  });
              ...
              // let updatedPlayer = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.update(playerObj.id,{ cash: newChips });
		   ...
		   if (player) {
			  // //console.log('<=> Removing Player || GAME-NUMBER [] ||');
			   ...
			   if (room.game && room.game.status == 'Running') {
				   ...
				   room  = await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.checkTurnFinished(room,playerId);
		   ...
		   await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('PlayerLeft', { 'playerId': player.id });
		...
		try {
			...
			for (var i = 0; i < room.players.length; i++) {
				if(room.players[i].status != 'Left'){
					...
					// await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('PlayerInfo', playerInfoObj);
				}
			}
    for (var i = 0; i < 6; i++) {
      let seatMatch = false;
    ...
    // dataArr.push({vacantSeat: playerFreeSeats});
		await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('playerInfoDummy', playerInfoDummy,{vacantSeat: playerFreeSeats});
		return room;
	 }catch (error) {
	...
	checkTurnFinished: async function(room,playerId) {
		...
		if (room.getCurrentPlayer().id == playerId) {
			...
			if(playinPlayer == 1){
				...
				await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.gameFinished(room);
						...
						if( timer < 1){
							...
							Sys.Timers[room.id] =  setInterval(async function(room){
								...
								if(timer < 1){
										clearTimeout(Sys.Timers[room.id]); // Clear Turn Timer
										//console.log("Turn - check is timer not Close...");
										await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.playerDefaultAction(room.id);
	...
	gameFinished : async function(room) {
	  	try {
	  		//console.log("GAME_FINESHED_ROOM_START :");
	  		//console.log("************************ :");
	  		//console.log("GAME_FINESHED_ROOM :",room);
	  		//console.log("************************ :");
				...
				// }
	  			if(playinPlayer != 1){
	  				...
	  				for (let i=0; i < room.game.declare.length; i += 1) {
	  					if(winnerPlayers.length == 0){
				            ...
				            if ( parseInt(FirstWinner.jokercardnumber) > 0) {
				            	valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+FirstWinner.jokercardnumber]);
				            }
				            if ( parseInt(FirstWinner.uppercardnumber) > 0) {
				            	valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+FirstWinner.uppercardnumber]);
				            }
				            if ( parseInt(FirstWinner.lowercardnumber) > 0) {
				            	valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+FirstWinner.lowercardnumber]);
				            }
				            if ( parseInt(FirstWinner.superuppercardnumber) > 0) {
				            	valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+FirstWinner.superuppercardnumber]);
				            }
				            if ( parseInt(FirstWinner.superlowercardnumber) > 0) {
				            	valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+FirstWinner.superlowercardnumber]);
				            }
				            if ( parseInt(FirstWinner.marriageindex) > 0) {
				            	valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['marrigeScore_24_'+FirstWinner.marriageindex]);
				            }
				            if ( parseInt(FirstWinner.superioindex) > 0) {
				            	valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['superioScore_24_'+FirstWinner.superioindex]);
				            }
				            if ( parseInt(FirstWinner.valuecards) > 0) {
				            	//console.log("");
				            	valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy.valueCardScore_24 * FirstWinner.valuecards);
				            }
	  						winnerPlayers.push({
		  						type            : 'winner',
							...
							// let playerObj = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.getById(LosserPlayer.id);
							...
							// let updatedPlayer = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.update(playerObj.id,{ chips: newChips });
							// Push Looser
							...
							// 	status           : 'sucess',
							// }
							// let chipsTransection = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.cerateChipsCashHistory(trnsObj);
				            ...
				            if (LosserPlayer.dropped != true) {
				              if ( parseInt(LosserPlayer.jokercardnumber) > 0) {
				                valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+LosserPlayer.jokercardnumber]);
				              }
				              if ( parseInt(LosserPlayer.uppercardnumber) > 0) {
				                valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+LosserPlayer.uppercardnumber]);
				              }
				              if ( parseInt(LosserPlayer.lowercardnumber) > 0) {
				                valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+LosserPlayer.lowercardnumber]);
				              }
				              if ( parseInt(LosserPlayer.superuppercardnumber) > 0) {
				                valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+LosserPlayer.superuppercardnumber]);
				              }
				              if ( parseInt(LosserPlayer.superlowercardnumber) > 0) {
				                valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+LosserPlayer.superlowercardnumber]);
				              }
				              if ( parseInt(LosserPlayer.marriageindex) > 0) {
				                valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['marrigeScore_24_'+LosserPlayer.marriageindex]);
				              }
				              if ( parseInt(LosserPlayer.superioindex) > 0) {
				              valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['superioScore_24_'+LosserPlayer.superioindex]);
				              }
				              if ( parseInt(LosserPlayer.valuecards) > 0) {
				                valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy.valueCardScore_24 * LosserPlayer.valuecards);
				              }
				            }
		  					winnerPlayers.push({
			  					type                     : 'loser',
						...
						//   // let adminCommissonAmount  = parseFloat( parseFloat( parseFloat( winnerPlayers[0].won ) * room.rack ) /100 );
						//   let updatedAdminCommission = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.createCommission({
						//     roomid                : room.id,
	  					...
	  					if (room.game.declare.indexOf(room.players[i].id) == -1 && room.players[i].status != 'waiting' ) { // if Player not in Declare Array
							...
							if (room.players[i].dropped != true) {
								if ( parseInt(room.players[i].jokercardnumber) > 0) {
									valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+room.players[i].jokercardnumber]);
								}
								if ( parseInt(room.players[i].uppercardnumber) > 0) {
									valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+room.players[i].uppercardnumber]);
								}
								if ( parseInt(room.players[i].lowercardnumber) > 0) {
									valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+room.players[i].lowercardnumber]);
								}
								if ( parseInt(room.players[i].superuppercardnumber) > 0) {
									valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+room.players[i].superuppercardnumber]);
								}
								if ( parseInt(room.players[i].superlowercardnumber) > 0) {
									valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+room.players[i].superlowercardnumber]);
								}
								if ( parseInt(room.players[i].marriageindex) > 0) {
									valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['marrigeScore_24_'+room.players[i].marriageindex]);
								}
								if ( parseInt(room.players[i].superioindex) > 0) {
									valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['superioScore_24_'+room.players[i].superioindex]);
								}
								if ( parseInt(room.players[i].valuecards) > 0) {
									valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy.valueCardScore_24 * room.players[i].valuecards);
								}
							}
			        ...
			        // }
		  			...
		  			//winnerPlayers[0].won
		  			// let playerObj = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.getById(winnerPlayers[0].playerId);
		  			// let newChips = parseFloat(playerObj.chips)+parseFloat(winnerPlayers[0].won);
		  			// let updatedPlayer = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.update(winnerPlayers[0].playerId,{ chips: newChips });
		  			...
		  			// let chipsTransection = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.cerateChipsCashHistory(trnsObj);
				        ...
				        if ( parseInt(winner.jokercardnumber) > 0) {
				          valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+winner.jokercardnumber]);
				        }
				        if ( parseInt(winner.uppercardnumber) > 0) {
				          valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+winner.uppercardnumber]);
				        }
				        if ( parseInt(winner.lowercardnumber) > 0) {
				          valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+winner.lowercardnumber]);
				        }
				        if ( parseInt(winner.superuppercardnumber) > 0) {
				          valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+winner.superuppercardnumber]);
				        }
				        if ( parseInt(winner.superlowercardnumber) > 0) {
				          valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+winner.superlowercardnumber]);
				        }
				        if ( parseInt(winner.marriageindex) > 0) {
				          valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['marrigeScore_24_'+winner.marriageindex]);
				        }
				        if ( parseInt(winner.superioindex) > 0) {
				        valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['superioScore_24_'+winner.superioindex]);
				        }
				        if ( parseInt(winner.valuecards) > 0) {
				          valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy.valueCardScore_24 * winner.valuecards);
				        }
			  			...
			  			// let playerObj = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.getById(winner.id);
			  			// let newChips = parseFloat(playerObj.chips)+parseFloat(singlePlayerWinnigchips) ;
			  			// let updatedPlayer = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.update(winner.id,{ chips: newChips });
						        ...
						        if (plrObj == false) {
						          ...
						          if (room.gameLosers[i].dropped != true) {
						            if ( parseInt(room.gameLosers[i].jokercardnumber) > 0) {
						              valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+room.gameLosers[i].jokercardnumber]);
						            }
						            if ( parseInt(room.gameLosers[i].uppercardnumber) > 0) {
						              valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+room.gameLosers[i].uppercardnumber]);
						            }
						            if ( parseInt(room.gameLosers[i].lowercardnumber) > 0) {
						              valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+room.gameLosers[i].lowercardnumber]);
						            }
						            if ( parseInt(room.gameLosers[i].superuppercardnumber) > 0) {
						              valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+room.gameLosers[i].superuppercardnumber]);
						            }
						            if ( parseInt(room.gameLosers[i].superlowercardnumber) > 0) {
						              valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['cardScore_24_'+room.gameLosers[i].superlowercardnumber]);
						            }
						            if ( parseInt(room.gameLosers[i].marriageindex) > 0) {
						              valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['marrigeScore_24_'+room.gameLosers[i].marriageindex]);
						            }
						            if ( parseInt(room.gameLosers[i].superioindex) > 0) {
						            valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy['superioScore_24_'+room.gameLosers[i].superioindex]);
						            }
						            if ( parseInt(room.gameLosers[i].valuecards) > 0) {
						              valueCardScore = parseFloat(valueCardScore) + parseFloat(Sys.Config.Rummy.valueCardScore_24 * room.gameLosers[i].valuecards);
						            }
						          }
				...
				for (var j = 0; j < winnerPlayers.length; j++) {
	      		...
	      		for (var k = 0; k < winnerPlayers.length; k++) {
	        		if(winnerPlayers[k].type == 'winner'){
						...
						// //console.log(":::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::");
						// Credit winner Chips
						let playerObj = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.getById(winnerPlayers[k].playerId);
						// TWENTYONE_CHANGE
						...
						if (parseFloat(winnerPlayers[k].won) > 0) {
	          			...
	          			//TDS Start
						let tds  = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.getSettingsData({});
						if (parseFloat(winnerPlayers[k].won) > tds.amount) {
							...
							let updatedAdminCommission  = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.createTds({
								roomid                : room.id,
								gameid                : room.game.id,
								gameType              : room.gameType,
							...
							let updatedTds = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.update(playerObj.id,{
								tdsAmount : totTds
							});
						...
						let playerCashManager = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.getCashManager({ playerId: winnerPlayers[k].playerId });
						let transferAmount = 0; //from lockCash to depositedCash
						let newLockCash = playerCashManager.lockCash;
						//calculate winning reward points
						...
						let playerDepositManager = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.getLastDepositManager({ playerId: playerObj.id, status: "Success" });
							...
							if (playerDepositManager.length != 0) {
								if ( playerDepositManager.requiredRewardPointReached != true ) {
									...
									if ( parseFloat(playerCashManager.rewardPoint + winningRewardPoint) >= playerDepositManager.requiredRewardPoint ) {
										...
										await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.updateDepositManager({ _id: playerDepositManager._id }, { requiredRewardPointReached : true });
									}
								}else {
						...
						let updatedPlayerCash = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.updateCashManager({ playerId: winnerPlayers[k].playerId },cashManageData);

						let updatedPlayer = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.update(playerObj.id,{ cash: newTotalCash, rewardPoint: playerCashManager.rewardPoint + winningRewardPoint, cashTransaction:(parseFloat(playerObj.cashTransaction) + parseFloat(winnerPlayers[k].won))  });
						// ~
		    			...
		    			let chipsTransection = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.cerateChipsCashHistory(trnsObj);
							...
							let newTotalCash = 0;
	          				if (LosserPlayer && LosserPlayer.status != 'Left') {
					            ...
					            let updatedPlayer = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.update(playerObj.id,{ cash: newTotalCash, rewardPoint: playerCashManager.rewardPoint + winningRewardPoint, cashTransaction:(parseFloat(playerObj.cashTransaction) + parseFloat(winnerPlayers[k].won*(-1)) )  });
					            // ~
	  			...
	  			Sys.Timers[room.id] = setInterval(async function(room){
			        ...
			        //   }
			        // }
		  			await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('OnGameFinished', {
		  			...
		  			if(timer < 1){
		  				...
		  				await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('OnGameRestart', {});
		  				for (i = 0; i < room.players.length; i++) {
		  					if(room.players[i].status != 'Left'){
						...
						Sys.Timers[room.id] = setInterval(async function(room){
							...
							if(timer < 1){
								...
								clearTimeout(Sys.Timers[room.id]); // Clear Room Timer
			                for (var i = 0; i < room.players.length; i++) {
			                  let player = await Sys.Game.CashGame.CashTwentyFour.Services.PlayerServices.getById(room.players[i].id);
			                  //console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
			                  //console.log("Checking the balance of players so to remove bankrupt");
			                  //console.log("room.players[i].chips", player.cash);
			                  //console.log("parseFloat( parseFloat(room.pointsValue)*130 )", parseFloat( parseFloat(room.pointsValue)*130 ));
			                  let feesdata = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.getData({gameType:'points_24'});
			                  if (parseFloat(player.cash) < parseFloat( parseFloat( parseFloat(room.pointsValue) * feesdata[0].entryFees ))) {
			                    ...
			                    await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.leaveRoom(playerData);
			            				// await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.leaveRoom(playerData);
	...
	pushOpenDeck: async function(data) {
		try{
			...
			if (room.game.status == 'Running') {
				...
				await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('PushOpenDeck',broObj);
	...
	popOpenDeck: async function(data) {
        ...
        // if(room.game.openDeck.length !=1 && room.game.openDeck[room.game.openDeck.length-1].length == 3){
				...
				await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('PopOpenDeck',broObj);
				//Save History
				let dataObj = {
					playerId:currentPlayer.id,
	...
	popCloseDeck: async function(data) {
		...
		//console.log("room.game.status->",room.game.status);
		 // await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('CloseDeckCardsRefill',{});
		...
		if (room.game.status == 'Running') {
			...
			await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('PopCloseDeck',broObj);
	...
	playerDrop: async function(data) {
		try{
			...
			await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('PlayerDrop', { 'playerId': data.playerId });

				roomUpdated = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.update(room); // Update Player

				//room  =  await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.broadcastPlayerInfo(room);
				room  =  await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.checkTurnFinished(room,data.playerId);
	...
	playerCardsScore: async function(data) {
		try{
          ...
          if (data.superuppercardnumber) {
            room.players[i].superuppercardnumber    = parseInt(data.superuppercardnumber);
          }
          if (data.superlowercardnumber) {
            room.players[i].superlowercardnumber    = parseInt(data.superlowercardnumber);
          }
  ...
  playerCards: async function(data) {
	...
	declarefinishGame: async function(data) {
		...
		if (room.game.status == 'Running') {
			...
			await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('PushFinishDeck',broObj);
			roomUpdated = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.update(room);
			...
			Sys.Timers[room.id] = setInterval(async function(room){
				...
				await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('OnFinishTimer', {
					roomId: room.id,
					timer : timer,
					playerId : data.playerId,
	...
	declareGame: async function(data) {
		...
		if (room.game.status == 'Running') {
			...
			room = await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.saveHistory(room,dataObj);
				...
				//console.log("currentPlayer.playerScore ++++++++++>",currentPlayer.playerScore);
					if(currentPlayer.playerScore == 0){
						...
						roomUpdated = await Sys.Game.CashGame.CashTwentyFour.Services.RoomServices.update(room); // Update room

						if(room.game.declare.length == 1){ // if Some one Already declare so not call declare
							room  =  await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.waitForGameDeclare(room,data.playerId);
						}
						...
						await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('CloseFinishTimer', {});
						currentPlayer.dropped = true;
						currentPlayer.playerScore = 130;
            if(currentPlayer.turnCount == 2 || currentPlayer.turnCount == 1){
              currentPlayer.playerScore = Sys.Config.Rummy.playerFirstDrop_24;
            }else if(currentPlayer.turnCount == 3){
              currentPlayer.playerScore = Sys.Config.Rummy.playerSecondDrop_24;
            }else{
              currentPlayer.playerScore = Sys.Config.Rummy.playerThirdDrop_24;
            }
			...
			// Other Player Decalre
				...
				if(allDeclare){
					clearTimeout(Sys.Timers[room.id]); // Clear waitforDeclare
					room  =  await Sys.Game.CashGame.CashTwentyFour.Controllers.RoomProcess.gameFinished(room);
				}
	...
	waitForGameDeclare: async function(room,playerId) {
		...
		Sys.Timers[room.id] = setInterval(async function(room){
			...
			await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(room.id).emit('OnDeclareTimer', {
				roomId: room.id,
				timer : timer,
				playerId : playerId,
  ...
  //       }
	...
	// 			await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(socket.id).emit('JokerOpenCardInfo', joObj);
  //
  //
	// 			room.players.forEach(async function (player) {
	// 				let playerCards = {
	...
	// 					cards : player.cards,
	// 				};
	// 				await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(socket.id).emit('ReconnectPlayerDeck', playerCards);
	// 			});
	// 			await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(socket.id).emit('TurnPlayer', {
	// 				playerId: room.getCurrentPlayer().id
	// 			});
	// 		}
  //
  ...
  reconnectGame: async function(socket,data){
      ...
      // if(room.game && room.game.status == 'Running'){
        ...
        room.players.forEach(async function (player) {
          ...
          await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(socket.id).emit('OnGameStartWait', {
            roomId: room.id,
            timer : timer,
        ...
        room.players.forEach(async function (player) {
          ...
          if(room.status=='Running' && player.id == data.playerId )
          {
            if ( player.turnCount < 2 ) {
              ...
              await Sys.Io.of(Sys.Config.Namespace.CashPointsTwentyFour).to(socket.id).emit('PlayerDeck', playerCards);
            } else {
