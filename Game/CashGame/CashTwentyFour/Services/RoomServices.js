...
module.exports = {
    ...
    create: async function(data){
        ...
        try {
            let tableCount = await roomModel.countDocuments({'gameType': 'cash_points24'});
            ...
            // room.id = room._id
            if(room){
                Sys.Rooms[room.id] = new Sys.Game.CashGame.CashTwentyFour.Entities.Room().createObject(room);
	...
	getSettingsData: async function(data){
     ...
     getData: async function(data){
        try {
          ...
          let d =  await entryFeesSettingsModel.find(data);
          //console.log("d",d)
          return d;
        } catch (e) {
          //console.log("Error",e);
